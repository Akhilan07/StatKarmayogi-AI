...
export const SplitScreenRagViewer: React.FC = () => {
  ...
  const handleGenerateFromChunk = () => {
    ...
    setTimeout(() => {
      ...
      setGeneratedMcq({
        ...
        similarityScore: selectedChunk.cosineSimilarity
      });
    }, 600);
  };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10 font-sans">
      {/* Header Banner */}
      <div className="bg-[#0a192f] text-white rounded-3xl p-6 sm:p-7 shadow-xl border border-cyan-900/60 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 font-mono text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
          ...
          <p className="text-xs sm:text-sm text-cyan-100/80 mt-1 max-w-xl">
            Inspect source MoSPI document chunks on the left while generating real-time grounded MCQs on the right.
          </p>
        </div>

        <div className="flex items-center gap-2 bg-slate-900/90 border border-cyan-500/30 px-3.5 py-2 rounded-xl text-xs text-cyan-300 font-mono">
          <Layers className="w-4 h-4 text-cyan-400" />
          <span>Chunk Embeddings: Cosine Sim &gt; 0.85</span>
        </div>
      </div>

      {/* Split-Screen Dual-Pane Layout */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Pane (Span 6): PDF / Manual Text Document & Chunk Reader */}
        <section className="lg:col-span-6 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-5">
          <div className="flex justify-between items-center pb-3 border-b border-slate-100">
            ...
            <span className="text-[11px] font-mono text-cyan-700 bg-cyan-50 px-2.5 py-1 rounded-md font-bold">
              {SAMPLE_RAG_DOCUMENT.chunks.length} Retrieved Chunks
            </span>
          </div>

          {/* Chunk Selector Tabs */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-700 block">Select Document Chunk:</label>
            <div className="space-y-2">
              {SAMPLE_RAG_DOCUMENT.chunks.map((chunk) => (
                <div
                  key={chunk.id}
                  onClick={() => {
                  ...
                  className={`p-3.5 rounded-xl border transition-all cursor-pointer ${
                    selectedChunkId === chunk.id
                      ? 'bg-cyan-50/80 border-cyan-500 shadow-xs ring-1 ring-cyan-500/30'
                      : 'bg-slate-50 border-slate-200 hover:bg-slate-100'
                  }`}
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-bold text-slate-900">{chunk.sectionTitle}</span>
                    <span className="text-[10px] font-mono font-bold text-cyan-700 bg-white px-2 py-0.5 rounded border border-cyan-200">
                      Sim: {chunk.cosineSimilarity}
                    </span>
                  </div>
          ...
          {/* Selected Chunk Highlight Box */}
          <div className="bg-slate-900 text-white rounded-2xl p-5 border border-slate-800 space-y-3">
            <div className="flex justify-between items-center text-xs">
              <span className="font-mono text-cyan-400 font-bold">{selectedChunk.chapter}</span>
              <span className="text-[10px] text-slate-400 font-mono">{selectedChunk.tokens} tokens</span>
            </div>
        ...
        <section className="lg:col-span-6 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-5">
          <div className="flex justify-between items-center pb-3 border-b border-slate-100">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500">Real-time prompt generation from chunk</p>
              </div>
            </div>
            ...
            <button
              ...
              className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold rounded-xl transition-all shadow-sm flex items-center gap-1.5 disabled:opacity-75"
            >
          ...
          {generatedMcq ? (
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="bg-cyan-50/60 border border-cyan-200 rounded-2xl p-5 space-y-3">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-cyan-900">Grounded Question Output</span>
                  <span className="text-[10px] font-mono font-bold bg-white text-cyan-700 px-2 py-0.5 rounded border border-cyan-200">
                    Similarity Match: {generatedMcq.similarityScore}
                  </span>
                </div>
              ...
              <div className="p-4 bg-slate-900 text-slate-200 rounded-2xl border border-slate-800 space-y-1.5 text-xs font-mono">
                <span className="text-cyan-400 font-bold block uppercase tracking-wider text-[10px]">Grounding Citation</span>
            ...
            <div className="p-10 text-center space-y-3 border-2 border-dashed border-slate-200 rounded-2xl">
              ...
              <p className="text-xs text-slate-500 max-w-xs mx-auto">
                Click "Generate Live MCQ" to extract a verified multiple-choice question directly from the selected chunk.
              </p>
            </div>
          )}
        </section>
